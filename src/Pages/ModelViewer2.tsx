...
const ModelViewer: FC<ModelViewerProps> = ({ initialModelPath = '/assets/Mezzanine_updated.glb' }) => {
  ...
  const [activeModelButton, setActiveModelButton] = useState<string>("withMezzanine");
  ...
  const [currentVariant, setCurrentVariant] = useState<'with' | 'without'>('with');
  ...
  const checkIntersection = () => {
    // Always allow intersection checking, but handle auto-rotation differently
    if (!cameraRef.current || targetMeshesRef.current.size === 0) {
      return;
    }
...
const handleMeshClick = () => {
  ...
  clickTimeoutRef.current = window.setTimeout(() => {
    ...
    if (unitNumber) {
      navigate(`/individual/${currentVariant}/${unitNumber}`);
    }
...
const lastModelChangeTimeRef = useRef<number>(0);
  ...
  const setupMouseEvents = () => {
    ...
    const onMouseMove = (event: MouseEvent) => {
      ...
      // During auto-rotation, force an intersection check immediately
      // to make highlighting more responsive
